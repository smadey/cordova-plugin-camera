...
+ (instancetype) createFromTakePictureArguments:(CDVInvokedUrlCommand*)command
{
    ...
    pictureOptions.hasOverlay = [[command argumentAtIndex:12 withDefault:@(NO)] boolValue];
...
@property (readwrite, assign) UIImageView *overlayHighlightImageView;
...
- (void)takePicture:(CDVInvokedUrlCommand*)command
{
    ...
    [self.commandDelegate runInBackground:^{
        ...
        if (pictureOptions.hasOverlay) {
            // Overlay start

            CGRect screenBounds = [[UIScreen mainScreen] bounds];
            CGRect bounds = CGRectMake(0, 44, screenBounds.size.width, screenBounds.size.width * 1.3333);

            UIView *overlayView = [[UIView alloc] initWithFrame:bounds];

            // 高亮层
            UIImageView *overlayHighlightImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"overlay-highlight.png"]];
            overlayHighlightImageView.frame = CGRectMake((bounds.size.width - overlayHighlightImageView.frame.size.width) / 2.0, (bounds.size.height - overlayHighlightImageView.frame.size.height) / 2.0, overlayHighlightImageView.frame.size.width, overlayHighlightImageView.frame.size.height);

            [overlayView addSubview:overlayHighlightImageView];
            self.overlayHighlightImageView = overlayHighlightImageView;

            // 示例图片
            UIImageView *overlayExampleImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"overlay-example.png"]];
            overlayExampleImageView.frame = CGRectMake((bounds.size.width - overlayExampleImageView.frame.size.width) / 2.0, overlayHighlightImageView.frame.origin.y - overlayExampleImageView.frame.size.height - 10, overlayExampleImageView.frame.size.width, overlayExampleImageView.frame.size.height);

            [overlayView addSubview:overlayExampleImageView];

            // 设置上下左右遮罩层
            UIView *overlayTopView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, overlayView.frame.size.width, overlayHighlightImageView.frame.origin.y)];
            overlayTopView.backgroundColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:0.7];
            [overlayView insertSubview:overlayTopView atIndex:0];

            UIView *overlayBottomView = [[UIView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(overlayHighlightImageView.frame), overlayView.frame.size.width, overlayView.frame.size.height - CGRectGetMaxY(overlayHighlightImageView.frame))];
            overlayBottomView.backgroundColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:0.7];
            [overlayView insertSubview:overlayBottomView atIndex:0];

            UIView *overlayLeftView = [[UIView alloc] initWithFrame:CGRectMake(0, overlayHighlightImageView.frame.origin.y, overlayHighlightImageView.frame.origin.x, overlayHighlightImageView.frame.size.height)];
            overlayLeftView.backgroundColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:0.7];
            [overlayView insertSubview:overlayLeftView atIndex:0];

            UIView *overlayRightView = [[UIView alloc] initWithFrame:CGRectMake(CGRectGetMaxX(overlayHighlightImageView.frame), overlayHighlightImageView.frame.origin.y, overlayView.frame.size.width - CGRectGetMaxX(overlayHighlightImageView.frame), overlayHighlightImageView.frame.size.height)];
            overlayRightView.backgroundColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:0.7];
            [overlayView insertSubview:overlayRightView atIndex:0];
...
- (UIImage*)retrieveImage:(NSDictionary*)info options:(CDVPictureOptions*)options
{
    ...
    if (options.hasOverlay) {
        CGRect screenBounds = [[UIScreen mainScreen] bounds];
        CGFloat scale = image.size.width / screenBounds.size.width;

        CGRect cropRect = CGRectMake(self.overlayHighlightImageView.frame.origin.x * scale, self.overlayHighlightImageView.frame.origin.y * scale, self.overlayHighlightImageView.frame.size.width * scale, self.overlayHighlightImageView.frame.size.height * scale);
    ...
    return (croppedImage == nil ? (scaledImage == nil ? image : scaledImage) : croppedImage);
}
